...
class ItemStoreDao {
  ...
  Future<int> upsert(ItemStore is_) async {
    ...
    if (existing == null) {
      // QUALITY (Finding 17): Added isEmpty guard before rows.first to avoid
      // a StateError if the row is missing for any reason (race, FK edge
      // case). Previously this would crash the entire upsert flow.
